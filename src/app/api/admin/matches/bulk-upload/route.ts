  ...
  try {
    ...
    if (!Array.isArray(matches) || matches.length === 0) {
      return NextResponse.json(
        { success: false, message: "matches array is required and must not be empty" },
        { status: 400 }
      );
    }
    ...
    for (let i = 0; i < matches.length; i++) {
      ...
      try {
        ...
        const matchType = (row.matchType || "NORMAL").toUpperCase() as MatchType;
        ...
        const defaults = MATCH_TYPE_DEFAULTS[matchType] || MATCH_TYPE_DEFAULTS.NORMAL;
